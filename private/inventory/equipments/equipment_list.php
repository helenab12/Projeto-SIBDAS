...
try {
    ...
    $stmtComponentes = execute_query(
        "SELECT c.idComponente, c.descricao, c.stock, GROUP_CONCAT(cc.idCategoria) as categorias_ids 
         FROM Componente c 
         LEFT JOIN ComponenteCategoria cc ON c.idComponente = cc.idComponente 
         WHERE c.ativo = 1 
         GROUP BY c.idComponente, c.descricao, c.stock
                        ...
                        <table id="equipmentsTable" class="heba-table w-100 display datatable-table">
                            ...
                            <tbody>
                                <?php foreach ($listaEquipamentos as $equipamento): ?>
                                    <?php
                                    // Encriptar ID
                                    $encryptedEqId = aes_encrypt($equipamento->getIdEquipamento());

                                    // Obter nome e descrição da categoria
        ...
        <div class="modal-content custom-modal-content d-flex flex-column">
            ...
            <div class="modal-body p-0">
                ...
                <form id="equipment-creation-form" action="equipments-crud/create-equipment.php" method="POST"
                    enctype="multipart/form-data" class="d-flex flex-column h-100 m-0 w-100" novalidate>
                    <!-- Conteúdo Página 1 -->
                    <div id="modal-page-1" class="equipment-creation-modal-content padding-6 gap-6 d-flex flex-column">
                        ...
                        <div class="d-flex flex-column form-item w-100 mw-0">
                            ...
                            <div class="d-flex w-100 gap-2">
                                <select id="equipment-location" name="equipment-location" class="form-select mw-0"
                                    required>
                                ...
                                <!-- Link Criar Novo -->
                                <a href="<?= BASE_URL ?>private/entities/locations.php" target="_blank"
                                    class="btn btn-primary-outline btn-small w-auto text-nowrap gap-2 text-decoration-none">
                                    <!-- SVG External Link -->
                        ...
                        <?php if (SHOW_DEBUG_BUTTONS): ?>
                            <div class="d-flex flex-wrap gap-2 mt-4">
                                <button type="button" class="btn btn-primary-outline btn-small fw-700 flex-grow-1"
                                    onclick="const getOpt = (id) => { const s = document.getElementById(id); return s && s.options.length > 1 ? s.options[1].value : ''; }; prefillFields({'equipment-code': 'EQ-' + Math.floor(Math.random() * 10000), 'equipment-name': 'Ventilador Evita V500', 'equipment-model': 'Modelo Standard', 'equipment-serial': 'SN-' + Math.floor(Math.random() * 10000), 'equipment-purchase-date': '01/01/2023', 'equipment-manufacture-date': '01/01/2022', 'equipment-cost': '5000.00', 'equipment-notes': 'Equipamento gerado automaticamente para testes de debug.', 'equipment-entry-type': 'Compra', 'equipment-criticality': 'Alta', 'equipment-status': 'Ativo', 'equipment-category': getOpt('equipment-category'), 'equipment-brand': getOpt('equipment-brand'), 'equipment-location': getOpt('equipment-location')});">
                                    Ventilador (Alta Crit)
                                </button>
                                <button type="button" class="btn btn-primary-outline btn-small fw-700 flex-grow-1"
                                    onclick="const getOpt = (id) => { const s = document.getElementById(id); return s && s.options.length > 1 ? s.options[1].value : ''; }; prefillFields({'equipment-code': 'EQ-' + Math.floor(Math.random() * 10000), 'equipment-name': 'Bomba Infusora Alaris', 'equipment-model': 'Modelo Standard', 'equipment-serial': 'SN-' + Math.floor(Math.random() * 10000), 'equipment-purchase-date': '01/01/2023', 'equipment-manufacture-date': '01/01/2022', 'equipment-cost': '5000.00', 'equipment-notes': 'Equipamento gerado automaticamente para testes de debug.', 'equipment-entry-type': 'Doação', 'equipment-criticality': 'Média', 'equipment-status': 'Ativo', 'equipment-category': getOpt('equipment-category'), 'equipment-brand': getOpt('equipment-brand'), 'equipment-location': getOpt('equipment-location')});">
                                    Bomba (Média Crit)
                                </button>
                                <button type="button" class="btn btn-primary-outline btn-small fw-700 flex-grow-1"
                                    onclick="const getOpt = (id) => { const s = document.getElementById(id); return s && s.options.length > 1 ? s.options[1].value : ''; }; prefillFields({'equipment-code': 'EQ-' + Math.floor(Math.random() * 10000), 'equipment-name': 'Monitor Sinais Vitais X3', 'equipment-model': 'Modelo Standard', 'equipment-serial': 'SN-' + Math.floor(Math.random() * 10000), 'equipment-purchase-date': '01/01/2023', 'equipment-manufacture-date': '01/01/2022', 'equipment-cost': '5000.00', 'equipment-notes': 'Equipamento gerado automaticamente para testes de debug.', 'equipment-entry-type': 'Empréstimo', 'equipment-criticality': 'Baixa', 'equipment-status': 'Ativo', 'equipment-category': getOpt('equipment-category'), 'equipment-brand': getOpt('equipment-brand'), 'equipment-location': getOpt('equipment-location')});">
                                    Monitor (Baixa Crit)
                                </button>
                            </div>
                        <?php endif; ?>
                                        ...
                                        <div class="d-flex flex-column gap-3 padding-4 multi-select-form">
                                            ...
                                            <?php foreach ($componentesDisponiveis as $comp):
                                                ...
                                                <div class="d-flex align-items-start gap-3 w-100 multi-select-item <?= $stock <= 0 ? 'opacity-50' : '' ?>"
                                                    data-category-id="<?= htmlspecialchars($comp['categorias_ids'] ?? '') ?>">
                                ...
                                <div class="d-flex flex-column form-item w-100 mw-0">
                                    ...
                                    <div class="d-flex w-100 gap-2">
                                        <select id="equipment-location-<?= htmlspecialchars($encryptedEqId) ?>"
                                            name="equipment-location" class="form-select mw-0" required>
                                            ...
                                            <div class="d-flex flex-column form-item gap-2 w-100">
                                                <div class="d-flex flex-column gap-3 padding-4 multi-select-form">
                                                    ...
                                                    <?php foreach ($componentesDisponiveis as $comp):
                                                        ...
                                                        ?>
                                                        <?php
                                                        $catIds = explode(',', $comp['categorias_ids'] ?? '');
                                                        $showComponent = in_array((string)$equipamento->getIdCategoria(), $catIds) || $isChecked;
                                                        ?>
                                                        <div class="d-flex align-items-start gap-3 w-100 multi-select-item <?= ($disabled && !$isChecked) ? 'opacity-50' : '' ?>"
                                                            data-category-id="<?= htmlspecialchars($comp['categorias_ids'] ?? '') ?>"
                                                            style="<?= $showComponent ? '' : 'display: none !important;' ?>">
                                                            <div class="form-check m-0 pt-1">
                                                                <input class="form-check-input m-0" type="checkbox"
                                                                    name="equipment-components[]"
                                                                    value="<?= htmlspecialchars($comp['idComponente']) ?>"
